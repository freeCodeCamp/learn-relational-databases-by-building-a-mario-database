...
describe('The "sounds" table', () => {
  it('should have a column named "filename" with the correct properties', async () => {
    const query1 = `SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = 'public' AND is_nullable = 'NO' AND table_name = 'actions' AND column_name = 'action' AND data_type = 'character varying' AND character_maximum_length = 20;`;
    ...
    try {
      ...
      const uniqueColumn = res2.rows.findIndex(column => {
        return column.table_name === 'actions' && column.column_name === 'action';
      });
